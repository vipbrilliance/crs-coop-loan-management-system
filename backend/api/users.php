...
if ($method === 'POST') {
    ...
    if ($action === 'toggle-active' && $id) {
        ...
        audit_log($db, 'Users', 'UPDATED', 'User', (string)$id, $updatedUser['email'], ($next ? 'User reactivated.' : 'User deactivated.'), $updatedUser, null, null, 'HIGH');
    ...
    if ($action === 'reset-password' && $id) {
        ...
        $updatedUser = userRow($db, $id);
        audit_log($db, 'Users', 'UPDATED', 'User', (string)$id, $updatedUser['email'], 'Temporary password was generated.', ['user' => $updatedUser], null, null, 'HIGH');
    ...
    audit_log($db, 'Users', 'CREATED', 'User', (string)$newUser['id'], $newUser['email'], 'User account created with role ' . $role . '.', $newUser, null, null, 'HIGH');
    json_ok($newUser, 201);
}
...
if ($method === 'PUT' && $id) {
    ...
    $updatedUser = userRow($db, $id);
    audit_log($db, 'Users', 'UPDATED', 'User', (string)$id, $updatedUser['email'], 'User account updated.', ['changes' => $d, 'user' => $updatedUser], null, null, 'HIGH');
    json_ok($updatedUser);
}
